...
the voting proposal is emitted the network via the MLS application message with a lightweight,
...
When a MLS proposal message is created by the `steward`, a commit message follows
...
who received the commit message, the steward sends a welcome message to the new person
