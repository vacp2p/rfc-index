...
- **2026-05-27** — [`b7602ed`](https://github.com/logos-co/logos-lips/blob/b7602ed8a225d41ca0bfaaa432524dc84d2ded7e/docs/blockchain/deprecated/v1.1.0-bedrock-block-construction-alt.md) — chore: move blockchain specs from notion to github
...
| 1.1.0 | Removed service_rewards due to updated [\[1.1.0\] Service Reward Distribution Protocol](v1.1.0-bedrock-service-reward-distribution.md). Extended the [Block Execution](v1.1.0-bedrock-block-construction-alt.md#block-execution) logic with rewards distribution due to updated [\[1.1.0\] Service Reward Distribution Protocol](v1.1.0-bedrock-service-reward-distribution.md). Removed Block Samples subsection of the [Batch verification of ZK proofs](v1.1.0-bedrock-block-construction-alt.md#batch-verification-of-zk-proofs) from the [Annex](v1.1.0-bedrock-block-construction-alt.md#annex). Reordered the [Block Execution](v1.1.0-bedrock-block-construction-alt.md#block-execution) steps to enable immediate use of reward notes as inputs for transactions included in the proposal. | 2026-03-27 |
...
1. The block builder constructs a block proposal.
    ...
    1. The block builder selects valid Mantle Transactions (as defined in  [\[1.4.0\] Mantle](v1.4.0-mantle.md)) from its mempool and includes references to them in the proposal.
...
1. The validators execute the block proposal.
    1. They derive the new blockchain state from the previous one by executing transactions as defined in [\[1.4.0\] Mantle](v1.4.0-mantle.md).
    1. They update the different variables that need to be maintained over time.
    1. They execute the [\[1.2.1\] Service Reward Distribution Protocol](../raw/bedrock-service-reward-distribution.md) to generate reward notes locally.

# Constructions

## Hash

We are using two hashing algorithms that have the same output length of 256 bits (32 bytes) that are [\[1.0.2\] Common Cryptographic Components - Poseidon2 (ZK Friendly Hash Function)](../raw/common-cryptographic-components.md#poseidon2-zk-friendly-hash-function) and [\[1.0.2\] Common Cryptographic Components - BLAKE2b (General-Purpose Hashing)](../raw/common-cryptographic-components.md#blake2bgeneral-purpose-hashing).
...
- header is the header of the proposal; defined below: [Header](v1.1.0-bedrock-block-construction-alt.md#header).
- references is a set of 1024 references to transactions of a hash type; the size of the hash type is 32 bytes and is the transaction hash as defined in [\[1.4.0\] Mantle - Mantle Transaction](v1.4.0-mantle.md#mantle-transaction).
...
- bedrock_version is the version of the proposal message structure that supports other protocols defined in [\[1.1.0\]\[Overview\] Bedrock Architecture](../raw/bedrock-architecture-overview.md); its size is 1 byte and is fixed to 0x01.
...
- block_root is the root of the Merkle tree constructed from transaction hashes (defined in [\[1.4.0\] Mantle - Mantle Transaction](v1.4.0-mantle.md#mantle-transaction)) used for constructing the references list in the mempool_ransactions; the size of the hash is 32 bytes.
- proof_of_leadership is the proof confirming that the sender is the leader; defined below: [Proof of Leadership](v1.1.0-bedrock-block-construction-alt.md#proof-of-leadership).
...
Where mempool_transactions is a set of up to 1024 references to transactions of a zkhash type; the size of the zkhash type is 32 bytes and is the transaction hash as defined in [\[1.4.0\] Mantle - Mantle Transaction](v1.4.0-mantle.md#mantle-transaction).
...
1. Construct the mempool_transactions object:
    1. Select Mantle transactions:
        ...
        - Ensure each transaction:
            - Is valid according to [\[1.4.0\] Mantle](v1.4.0-mantle.md).
...
1. Assemble the block proposal.
    ...
    ```
    The PoL must have been generated beforehand and bound to the same Ledger view as mentioned in the [Prerequisites](v1.1.0-bedrock-block-construction-alt.md#prerequisites).
...
1. Mempool Transactions Validation
    mempool_transactions must refer to a valid sequence of Mantle Transactions from the mempool. Each transaction must be valid according to the rules defined in the [\[1.4.0\] Mantle](v1.4.0-mantle.md). In order to verify ZK proofs, they are batched for verification as explained in [Batch verification of ZK proofs](v1.1.0-bedrock-block-construction-alt.md#batch-verification-of-zk-proofs) to get better performance.
...
1. Execute the reward distribution protocol defined in [\[1.2.1\] Service Reward Distribution Protocol](../raw/bedrock-service-reward-distribution.md) to generate reward notes locally and include them in the ledger.
1. Execute the Mantle Transactions included in the block sequentially, using the execution rules defined in the [\[1.4.0\] Mantle](v1.4.0-mantle.md).
...
The verifier follows the same procedure as in [Proofs of Claim](v1.1.0-bedrock-block-construction-alt.md#proofs-of-claim) but with the Groth16 proofs of ZkSignatures.
