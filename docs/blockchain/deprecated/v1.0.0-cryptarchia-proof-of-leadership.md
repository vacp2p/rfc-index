...
| Field | Value |
| --- | --- |
| Name | [1.0.0] Proof of Leadership |
| Slug | 214 |
| Status | deprecated |
| Type | RFC |
...
- **2026-05-27** — [`b7602ed`](https://github.com/logos-co/logos-lips/blob/b7602ed8a225d41ca0bfaaa432524dc84d2ded7e/docs/blockchain/deprecated/v1.0.0-cryptarchia-proof-of-leadership.md) — chore: move blockchain specs from notion to github
...
- [\[1.0.1\] Cryptarchia Protocol](../raw/cryptarchia-v1-protocol.md).
...
- Setup: When refreshing their notes, potential leaders will:
    1. Uniformly randomly draw a seed $r_1 \overset{\$}{\leftarrow} \mathbb F_p$.
    ...
    1. Finally, they derive their secret key $sk := \text{hash}(\text{LOGOS\_POL\_SK\_V1}||sl_{start} || R)$, binding the starting slot and the Merkle tree of slot secret to the note secret key. This is verified in [Circuit Constraints](v1.0.0-cryptarchia-proof-of-leadership.md#circuit-constraints).
    ...
    return (zkhash(b"LOGOS_POL_SK_V1" + sl_start + root), path)
...
![Diagram](v1.0.0-cryptarchia-proof-of-leadership/assets/21c261aa-09df-81ac-91ec-eeb257b42989.png)
...
1. The epoch nonce: $\eta$.
    - For details see [\[1.0.1\] Cryptarchia Protocol - Epoch Nonce](../raw/cryptarchia-v1-protocol.md#epoch-nonce).
1. The lottery function constants: $t_0 = -\frac{\text{VRF}\_order \ln(1-f)}{\text{inferred\_total\_stake}}$ and $t_1=- \frac{\text{VRF}\_order\ln^2(1-f)}{2 \cdot \text{inferred\_total\_stake}^2}$.
    - For details see [Lottery Approximation](v1.0.0-cryptarchia-proof-of-leadership.md#lottery-approximation).
...
1. The root of the note Merkle tree when the stake distribution was frozen $ledger_\text{AGED}$.
    - For details see [\[1.0.1\] Cryptarchia Protocol - Epoch State Pseudocode](../raw/cryptarchia-v1-protocol.md#epoch-state-pseudocode).
...
1. The leader's one-time public key $P_\text{LEAD}$ represented by 2 public inputs, each of 16 bytes in little endian. This key is needed to sign the proposed block.
    - For details see [Linking the Proof of Leadership to a Block](v1.0.0-cryptarchia-proof-of-leadership.md#linking-the-proof-of-leadership-to-a-block).
1. The entropy contribution $\rho_{LEAD}$ verified to be correctly derived.
    - This is the epoch nonce entropy contribution. See [\[1.0.1\] Cryptarchia Protocol - Epoch Nonce](../raw/cryptarchia-v1-protocol.md#epoch-nonce).

### Circuit Private Inputs

The prover has to provide these values, but they remain secret:

1. The slot secret and the related information used for the slot $sl$ as described in [Protection Against Adaptive Adversaries](v1.0.0-cryptarchia-proof-of-leadership.md#protection-against-adaptive-adversaries):
    - The slot secret $r_{sl}$.
    - The Merkle path $slot\_secret\_path$ of $r_{sl}$ leading to the root $R$.
    - The starting secret slot $sl_{start}$.
1. The eligible note and its related information used to derive the [noteID](v1.0.0-mantle.md) (the secret key is derived for the previous step):
    - The note value: $v$.
...
1. The derivation of $sk = \text{hash}(\text{LOGOS\_POL\_SK\_V1}||sl_{start} || R)$, as documented in [Protection Against Adaptive Adversaries](v1.0.0-cryptarchia-proof-of-leadership.md#protection-against-adaptive-adversaries).
1. The computation of the note identifier.
1. The note identifier is in $ledger_{AGED}$ and $ledger_{LATEST}$.
1. The computation of the lottery ticket: $ticket := \text{hash}(\text{LEAD\_V1}||\eta||sl||noteID||sk)$ using [Poseidon2](../raw/common-cryptographic-components.md).
1. The computation of the threshold: $t:= v(t_0+t_1\cdot v)$.
    The ticket must be lower than this threshold to win the lottery.
1. The check that indeed $ticket \lt t$.
1. Compute and output the entropy contribution $\rho_{LEAD} := \text{hash}(\text{LOGOS\_NONCE\_CONTRIB\_V1} || sl||noteID||sk)$
...
![Diagram](v1.0.0-cryptarchia-proof-of-leadership/assets/240261aa-09df-80a0-ab15-ea46ea2d8bc8.png)
