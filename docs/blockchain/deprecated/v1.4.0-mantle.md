...
- **2026-05-27** — [`b7602ed`](https://github.com/logos-co/logos-lips/blob/b7602ed8a225d41ca0bfaaa432524dc84d2ded7e/docs/blockchain/deprecated/v1.4.0-mantle.md) — chore: move blockchain specs from notion to github
...
| 1.2.1 | Added [\[RFC\] Improve Mantle Transaction hash](../raw/mantle-transaction-encoding/appendices/rfc-improve-mantle-transaction-hash.md). |
| 1.3.0 | Added [\[RFC\] Make Ledger Transaction an Operation](../raw/mantle-transaction-encoding/appendices/rfc-make-ledger-transaction-an-operation.md). |
| 1.4.0 | Added [\[RFC\] Enforce NoteId uniqueness](../raw/mantle-transaction-encoding/appendices/rfc-enforce-noteid-uniqueness.md). |
...
The [hash function used](../raw/common-cryptographic-components.md), as well as other cryptographic primitives like ZK proofs and signature schemes, are described in [Common Cryptographic Components](../raw/common-cryptographic-components.md).
...
Channel Inscribe Operations have a fixed Execution Gas cost of EXECUTION_CHANNEL_INSCRIBE_GAS. See [Gas Determination](v1.4.0-mantle.md#gas-determination) for the Execution Gas values.
...
Channel Config Operations have a linear Execution Gas cost equal to EXECUTION_CHANNEL_CONFIG_GAS * configuration_threshold. See [Gas Determination](v1.4.0-mantle.md#gas-determination) for the Execution Gas values.
...
A Channel Deposit proves the ownership of the consumed notes using a [Zero Knowledge Signature Scheme (ZkSignature)](v1.4.0-mantle.md#zero-knowledge-signature-scheme-zksignature).
...
Channel Deposit Operations have a fixed Execution Gas cost of EXECUTION_CHANNEL_DEPOSIT_GAS. See [Gas Determination](v1.4.0-mantle.md#gas-determination) for the Execution Gas values.
...
Channel Withdraw Operations have a linear Execution Gas cost equal to EXECUTION_CHANNEL_WITHDRAW_GAS * withdraw_threshold. See [Gas Determination](v1.4.0-mantle.md#gas-determination) for the Execution Gas values.
...
These Operations implement the [Service Declaration Protocol](../raw/bedrock-service-declaration-protocol.md).
...
The service registration follows the definition given in [Service Declaration Protocol - Declaration Message](../raw/bedrock-service-declaration-protocol.md#declaration-message):
...
Locked notes are introduced in [Locked notes](v1.4.0-mantle.md#locked-notes) and serve as Service collaterals. They cannot be spent before the owner withdraw its participation from the declared service(s).
...
see: [Zero Knowledge Signature Scheme (ZkSignature)](v1.4.0-mantle.md#zero-knowledge-signature-scheme-zksignature).

Execution Gas

SDP Declare Operations have a fixed Execution Gas cost of EXECUTION_SDP_DECLARE_GAS. See [Gas Determination](v1.4.0-mantle.md#gas-determination) for the Execution Gas values.
...
The declaration is verified according to [Service Declaration Protocol - Declare](../raw/bedrock-service-declaration-protocol.md#declare).
...
1. Add this declaration to the locked note.
    ...
    ```
1. Store the declaration as explained in [Service Declaration Protocol - Declaration Storage](../raw/bedrock-service-declaration-protocol.md#declaration-storage).
    ```python
...
The service withdrawal follows the definition given in [Service Declaration Protocol - Withdraw Message](../raw/bedrock-service-declaration-protocol.md#withdraw-message).
...
A signature from the zk_id and the locked note pk attached to the declaration is required for withdrawing from a service, (see [Zero Knowledge Signature Scheme (ZkSignature)](v1.4.0-mantle.md#zero-knowledge-signature-scheme-zksignature)).
...
SDP Withdraw Operations have a fixed Execution Gas cost of EXECUTION_SDP_WITHDRAW_GAS. See [Gas Determination](v1.4.0-mantle.md#gas-determination) for the Execution Gas values.
...
1. Ensure that the locked note has expired.
    ...
    ```
1. Validate SDP withdrawal according to [Service Declaration Protocol - Withdraw](../raw/bedrock-service-declaration-protocol.md#withdraw).
...
Executes the withdrawal protocol [Service Declaration Protocol - Withdraw](../raw/bedrock-service-declaration-protocol.md#withdraw).
...
The service active action follows the definition given in [Service Declaration Protocol - Active Message](../raw/bedrock-service-declaration-protocol.md#active-message).
...
SDP Active Operations have a fixed Execution Gas cost of EXECUTION_SDP_ACTIVE_GAS. See [Gas Determination](v1.4.0-mantle.md#gas-determination) for the Execution Gas values.
...
Executes the active protocol [Service Declaration Protocol - Active](../raw/bedrock-service-declaration-protocol.md#active). The activation, i.e. setting the declaration.active, is handled by the service-specific logic.
...
The provider proves that they have won a proof of Leadership before the start of the current epoch, i.e., their reward voucher is indeed in the voucher set: [Proof of Claim](v1.4.0-mantle.md#proof-of-claim).

Execution gas

Leader Claim Operations have a fixed Execution Gas cost of EXECUTION_LEADER_CLAIM_GAS. See [Gas Determination](v1.4.0-mantle.md#gas-determination) for the Execution Gas values.
...
A Transfer proves the ownership of the consumed notes using a [Zero Knowledge Signature Scheme (ZkSignature)](v1.4.0-mantle.md#zero-knowledge-signature-scheme-zksignature).
...
Transfer have a fixed Execution Gas cost of EXECUTION_TRANSFER_GAS. See [Gas Determination](v1.4.0-mantle.md#gas-determination) for the Execution Gas values.
...
From the [\[Analysis\] Gas Cost Determination](v1.4.0-analysis-gas-cost-determination.md), we get the table below:
...
![Diagram](v1.4.0-mantle/assets/335261aa-09df-81f5-bcdd-ffbbfb337529.png)
...
![Diagram](v1.4.0-mantle/assets/335261aa-09df-81c4-917f-c4eaa16588bb.png)
