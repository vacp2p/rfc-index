...
1. Service Activity Tracking (Session N+1): Service validators submit signed activity messages to attest to their participation of session N through a Mantle Transaction, including an activity message (see [\[1.3.0\] Mantle - SDP_ACTIVE](v1.3.0-mantle.md#sdp_active)).
...
- Service rewards are distributed to the zk_id from validator [\[1.0.0\] Service Declaration Protocol - Declaration Message](../raw/bedrock-service-declaration-protocol.md#declaration-message)s.
...
Throughout session N+1, the block proposers integrate Mantle Transactions containing [\[1.3.0\] Mantle - SDP_ACTIVE](v1.3.0-mantle.md#sdp_active) Operations. These transactions originate from service validators and are used to derive their activity according to the service provided policy (see [\[1.0.0\] Service Declaration Protocol - Active](../raw/bedrock-service-declaration-protocol.md#active)). The protocol does not prescribe a unique activity rule: each service defines what qualifies as valid participation, enabling flexibility across different services.
...
- Be sent to the public key zk_id of the validator registered during declaration of the service (see [\[1.0.0\] Service Declaration Protocol - Declaration Message](../raw/bedrock-service-declaration-protocol.md#declaration-message)).
...
Nodes indirectly verify the correct inclusion of rewards because all consensus-validating nodes must maintain the same ledger view to derive the latest ledger root, which serves as input for verifying the [\[1.1.0\] Proof of Leadership](../raw/cryptarchia-proof-of-leadership.md).
