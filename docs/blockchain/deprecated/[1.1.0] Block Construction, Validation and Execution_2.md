# [1.1.0] Block Construction, Validation and Execution_2

| Field | Value |
| --- | --- |
| Name | [1.1.0] Block Construction, Validation and Execution_2 |
| Slug | 249 |
| Status | deprecated |
| Type | RFC |
...
- **2026-05-26** — [`52fe381`](https://github.com/logos-co/logos-lips/blob/52fe381c0a6cea3f7a5b075615409d90320e2543/docs/blockchain/deprecated/[1.1.0] Block Construction, Validation and Execution_2.md) — chore(blockchain): import 25 deprecated specs, rename 2 existing to [X.Y.Z] Title convention
...
| 1.1.0 | Removed service_rewards due to updated [[1.1.0] Service Reward Distribution Protocol](https://nomos-tech.notion.site/1-1-0-Service-Reward-Distribution-Protocol-26b261aa09df8032861dddf01182e242?pvs=24). Extended the [Block Execution](https://nomos-tech.notion.site/Block-Execution-269261aa09df807185a9e0764acffe22?pvs=24#269261aa09df8106b6afd9975a01fe2e) logic with rewards distribution due to updated [[1.1.0] Service Reward Distribution Protocol](https://nomos-tech.notion.site/1-1-0-Service-Reward-Distribution-Protocol-26b261aa09df8032861dddf01182e242?pvs=24). Removed Block Samples subsection of the [Batch verification of ZK proofs](https://nomos-tech.notion.site/Batch-verification-of-ZK-proofs-269261aa09df807185a9e0764acffe22?pvs=24#269261aa09df80f9a636d1c4751f48ab) from the [Annex](https://nomos-tech.notion.site/Annex-269261aa09df807185a9e0764acffe22?pvs=24#269261aa09df80669556fcac40ea7f8b). Reordered the [Block Execution](https://nomos-tech.notion.site/Block-Execution-269261aa09df807185a9e0764acffe22?pvs=24#269261aa09df8106b6afd9975a01fe2e) steps to enable immediate use of reward notes as inputs for transactions included in the proposal. | 2026-03-27 |
...
1. The block builder constructs a block proposal.
    ...
    1. The block builder selects valid Mantle Transactions (as defined in  [[1.4.0] Mantle](https://nomos-tech.notion.site/1-4-0-Mantle-335261aa09df8065a38acff4b25aee82?pvs=24)) from its mempool and includes references to them in the proposal.
...
1. The validators execute the block proposal.
    1. They derive the new blockchain state from the previous one by executing transactions as defined in [[1.4.0] Mantle](https://nomos-tech.notion.site/1-4-0-Mantle-335261aa09df8065a38acff4b25aee82?pvs=24).
    1. They update the different variables that need to be maintained over time.
    1. They execute the [[1.2.1] Service Reward Distribution Protocol](https://nomos-tech.notion.site/1-2-1-Service-Reward-Distribution-Protocol-341261aa09df809cbdb6e4a0b0314c4f?pvs=24) to generate reward notes locally.

# Constructions

## Hash

We are using two hashing algorithms that have the same output length of 256 bits (32 bytes) that are [[1.0.2] Common Cryptographic Components - Poseidon2 (ZK Friendly Hash Function)](https://nomos-tech.notion.site/Poseidon2-ZK-Friendly-Hash-Function-1fd261aa09df81ac8ebbe0111e2c2d84?pvs=24#1fd261aa09df81f48afcd5bbe86c4a18) and [[1.0.2] Common Cryptographic Components - BLAKE2b (General-Purpose Hashing)](https://nomos-tech.notion.site/BLAKE2b-General-Purpose-Hashing-1fd261aa09df81ac8ebbe0111e2c2d84?pvs=24#1fd261aa09df81b3890fcc4f9606ee9e).
...
- header is the header of the proposal; defined below: [Header](https://nomos-tech.notion.site/Header-269261aa09df807185a9e0764acffe22?pvs=24#269261aa09df817a9871d17ab86c0504).
- references is a set of 1024 references to transactions of a hash type; the size of the hash type is 32 bytes and is the transaction hash as defined in [[1.4.0] Mantle - Mantle Transaction](https://nomos-tech.notion.site/Mantle-Transaction-335261aa09df8065a38acff4b25aee82?pvs=24#335261aa09df8183b425e3ebb8705a89).
...
- bedrock_version is the version of the proposal message structure that supports other protocols defined in [[1.1.0][Overview] Bedrock Architecture](https://nomos-tech.notion.site/1-1-0-Overview-Bedrock-Architecture-2e0261aa09df80938c08cd51b9759788?pvs=24); its size is 1 byte and is fixed to 0x01.
...
- block_root is the root of the Merkle tree constructed from transaction hashes (defined in [[1.4.0] Mantle - Mantle Transaction](https://nomos-tech.notion.site/Mantle-Transaction-335261aa09df8065a38acff4b25aee82?pvs=24#335261aa09df8183b425e3ebb8705a89)) used for constructing the references list in the mempool_ransactions; the size of the hash is 32 bytes.
- proof_of_leadership is the proof confirming that the sender is the leader; defined below: [Proof of Leadership](https://nomos-tech.notion.site/Proof-of-Leadership-269261aa09df807185a9e0764acffe22?pvs=24#269261aa09df81aebc87d589573de606).
...
1. Construct the mempool_transactions object:
    1. Select Mantle transactions:
        ...
        - Ensure each transaction:
            - Is valid according to [[1.4.0] Mantle](https://nomos-tech.notion.site/1-4-0-Mantle-335261aa09df8065a38acff4b25aee82?pvs=24).
            - Has no conflicts with others (e.g., two transactions trying to spend the same note).
1. Derive references values:
    ```text
    references: list[zkhash] = [mantle_txhash(tx) for tx in mempool_transactions]
    ```
1. Compute the header.block_root as the root of the Merkle tree constructed from the mempool_transactions transactions used to build references.
...
1. Assemble the block proposal.
    ...
    ```
    The PoL must have been generated beforehand and bound to the same Ledger view as mentioned in the [Prerequisites](https://nomos-tech.notion.site/Prerequisites-269261aa09df807185a9e0764acffe22?pvs=24#269261aa09df81088033f9cbc7d456a6).
...
1. Mempool Transactions Validation
    mempool_transactions must refer to a valid sequence of Mantle Transactions from the mempool. Each transaction must be valid according to the rules defined in the [[1.4.0] Mantle](https://nomos-tech.notion.site/1-4-0-Mantle-335261aa09df8065a38acff4b25aee82?pvs=24). In order to verify ZK proofs, they are batched for verification as explained in [Batch verification of ZK proofs](https://nomos-tech.notion.site/Batch-verification-of-ZK-proofs-269261aa09df807185a9e0764acffe22?pvs=24#269261aa09df80f9a636d1c4751f48ab) to get better performance.
...
1. Execute the reward distribution protocol defined in [[1.2.1] Service Reward Distribution Protocol](https://nomos-tech.notion.site/1-2-1-Service-Reward-Distribution-Protocol-341261aa09df809cbdb6e4a0b0314c4f?pvs=24) to generate reward notes locally and include them in the ledger.
1. Execute the Mantle Transactions included in the block sequentially, using the execution rules defined in the [[1.4.0] Mantle](https://nomos-tech.notion.site/1-4-0-Mantle-335261aa09df8065a38acff4b25aee82?pvs=24).
...
The verifier follows the same procedure as in [Proofs of Claim](https://nomos-tech.notion.site/Proofs-of-Claim-269261aa09df807185a9e0764acffe22?pvs=24#269261aa09df808aa887e376efeec33d) but with the Groth16 proofs of ZkSignatures.
