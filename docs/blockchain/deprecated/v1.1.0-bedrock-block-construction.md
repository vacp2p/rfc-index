...
1. The block builder constructs a block proposal.
    ...
    1. The block builder selects valid Mantle Transactions (as defined in  [\[1.0.0\] Mantle](v1.0.0-mantle.md)) from its mempool and includes references to them in the proposal.
...
1. The validators execute the block proposal.
    1. They derive the new blockchain state from the previous one by executing transactions as defined in [\[1.0.0\] Mantle](v1.0.0-mantle.md).
...
- references is a set of 1024 references to transactions of a hash type; the size of the hash type is 32 bytes and is the transaction hash as defined in [\[1.0.0\] Mantle - Mantle Transaction](v1.0.0-mantle.md#mantle-transaction).
...
- parent_block is the block ID ([\[1.0.2\] Cryptarchia Protocol](../raw/cryptarchia-v1-protocol.md)) of the parent block, validated and accepted by the block builder. It is used for the derivation of the AgedLedger and LatestLedger values necessary for validating the PoL; the size of the hash is 32 bytes.
- slot is the consensus slot number; the size of the SlotNumber type is 8 bytes.
- block_root is the root of the Merkle tree constructed from transaction hashes (defined in [\[1.0.0\] Mantle - Mantle Transaction](v1.0.0-mantle.md#mantle-transaction)) used for constructing the references list in the transactions; the size of the hash is 32 bytes.
...
- service_reward is a set of up to 1 reference to a reward transaction of a zkhash type; the size of the zkhash type is 32 bytes and is the transaction hash as defined in [\[1.0.0\] Mantle - Mantle Transaction](v1.0.0-mantle.md#mantle-transaction).
- mempool_transactions is a set of up to 1024 references to transactions of a zkhash type; the size of the zkhash type is 32 bytes and is the transaction hash as defined in [\[1.0.0\] Mantle - Mantle Transaction](v1.0.0-mantle.md#mantle-transaction).
...
Only after the PoL is generated can the block proposal be constructed (see [\[1.1.0\] Proof of Leadership](../raw/cryptarchia-proof-of-leadership.md)).
...
1. Construct the mempool_transactions object:
    1. Select Mantle transactions:
        ...
        - Ensure each transaction:
            - Is valid according to [\[1.0.0\] Mantle](v1.0.0-mantle.md).
...
1. Block Validation
    The Proposal must satisfy the rules defined in [\[1.0.2\] Cryptarchia Protocol - Block Header Validation](../raw/cryptarchia-v1-protocol.md#block-header-validation).
...
1. Execute the Mantle Transactions included in the block sequentially, using the execution rules defined in the [\[1.0.0\] Mantle](v1.0.0-mantle.md).
