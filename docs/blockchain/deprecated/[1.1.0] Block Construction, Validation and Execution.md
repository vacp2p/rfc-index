# [1.1.0] Block Construction, Validation and Execution

| Field | Value |
| --- | --- |
| Name | [1.1.0] Block Construction, Validation and Execution |
| Slug | 248 |
| Status | deprecated |
| Type | RFC |
...
- **2026-05-26** — [`52fe381`](https://github.com/logos-co/logos-lips/blob/52fe381c0a6cea3f7a5b075615409d90320e2543/docs/blockchain/deprecated/[1.1.0] Block Construction, Validation and Execution.md) — chore(blockchain): import 25 deprecated specs, rename 2 existing to [X.Y.Z] Title convention
...
1. The block builder constructs a block proposal.
    ...
    1. The block builder constructs references to the deterministically generated Mantle Transactions that execute the [[1.0.0] Service Reward Distribution Protocol](https://nomos-tech.notion.site/1-0-0-Service-Reward-Distribution-Protocol-211261aa09df816d92c8c6ec00c6e227?pvs=24), if such transactions can be constructed. For example, there is no need to distribute rewards when all rewards have already been distributed.
    1. The block builder selects valid Mantle Transactions (as defined in  [[1.0.0] Mantle](https://nomos-tech.notion.site/1-0-0-Mantle-21c261aa09df810c8820fab1d78b53d9?pvs=24)) from its mempool and includes references to them in the proposal.
...
1. The validators validate the block proposal.
    ...
    1. They verify distribution of service rewards through Mantle Transactions as specified in [[1.0.0] Service Reward Distribution Protocol](https://nomos-tech.notion.site/1-0-0-Service-Reward-Distribution-Protocol-211261aa09df816d92c8c6ec00c6e227?pvs=24). This is done by independently deriving the distribution transaction and confirming that it matches the first reference, if there is rewards to be distributed.
...
1. The validators execute the block proposal.
    1. They derive the new blockchain state from the previous one by executing transactions as defined in [[1.0.0] Mantle](https://nomos-tech.notion.site/1-0-0-Mantle-21c261aa09df810c8820fab1d78b53d9?pvs=24).
...
We are using two hashing algorithms that have the same output length of 256 bits (32 bytes) that are [Poseidon2 and Blake2b](https://nomos-tech.notion.site/1fd261aa09df81ac8ebbe0111e2c2d84?pvs=25).
...
- header is the header of the proposal; defined below: [Header](https://nomos-tech.notion.site/Header-2e3261aa09df8008b50fc06a0730f7d4?pvs=24#2e3261aa09df80e0b7f4e8fe56802199).
- references is a set of 1024 references to transactions of a hash type; the size of the hash type is 32 bytes and is the transaction hash as defined in [[1.0.0] Mantle - Mantle Transaction](https://nomos-tech.notion.site/Mantle-Transaction-21c261aa09df810c8820fab1d78b53d9?pvs=24#21c261aa09df813a8395e89b61500298).
...
- bedrock_version is the version of the proposal message structure that supports other protocols defined in [No access](https://nomos-tech.notion.site/1fd261aa09df8101b69ff4999eea9c8c?pvs=24#1fd261aa09df8101b69ff4999eea9c8c); the size of it is 1 byte and is fixed to 0x01.
- parent_block is the block ID ([[1.0.2] Cryptarchia Protocol](https://nomos-tech.notion.site/1-0-2-Cryptarchia-Protocol-21c261aa09df810cb85eff1c76e5798c?pvs=24)) of the parent block, validated and accepted by the block builder. It is used for the derivation of the AgedLedger and LatestLedger values necessary for validating the PoL; the size of the hash is 32 bytes.
- slot is the consensus slot number; the size of the SlotNumber type is 8 bytes.
- block_root is the root of the Merkle tree constructed from transaction hashes (defined in [[1.0.0] Mantle - Mantle Transaction](https://nomos-tech.notion.site/Mantle-Transaction-21c261aa09df810c8820fab1d78b53d9?pvs=24#21c261aa09df813a8395e89b61500298)) used for constructing the references list in the transactions; the size of the hash is 32 bytes.
- proof_of_leadership is the proof confirming that the sender is the leader; defined below: [Proof of Leadership](https://nomos-tech.notion.site/Proof-of-Leadership-2e3261aa09df8008b50fc06a0730f7d4?pvs=24#2e3261aa09df80c5a403d156303e1b35).
...
- mempool_transactions is a set of up to 1024 references to transactions of a zkhash type; the size of the zkhash type is 32 bytes and is the transaction hash as defined in [[1.0.0] Mantle - Mantle Transaction](https://nomos-tech.notion.site/Mantle-Transaction-21c261aa09df810c8820fab1d78b53d9?pvs=24#21c261aa09df813a8395e89b61500298).
...
The service_reward transaction is a [Service Rewards Distribution Transaction](https://nomos-tech.notion.site/211261aa09df816d92c8c6ec00c6e227?pvs=25#211261aa09df81ff867ddbe98607fa56) that distributes service rewards. It is a Mantle Transaction with no input and up to service_count x 4 outputs, service_count being the number of services (global parameter). The outputs represent the validators rewarded (up to 4 per service).
...
Only after the PoL is generated can the block proposal be constructed (see [[1.1.0] Proof of Leadership](https://nomos-tech.notion.site/1-1-0-Proof-of-Leadership-2e9261aa09df80058244c902defc6da2?pvs=24)).
...
1. Construct the mempool_transactions object:
    1. Select Mantle transactions:
        ...
        - Ensure each transaction:
            - Is valid according to [[1.0.0] Mantle](https://nomos-tech.notion.site/1-0-0-Mantle-21c261aa09df810c8820fab1d78b53d9?pvs=24).
...
1. Assemble the block proposal.
    ...
    ```
    The PoL must have been generated beforehand and bound to the same Ledger view as mentioned in the [Prerequisites](https://nomos-tech.notion.site/Prerequisites-2e3261aa09df8008b50fc06a0730f7d4?pvs=24#2e3261aa09df803bbaf2d5e996f61781).
...
1. Block Validation
    The Proposal must satisfy the rules defined in [[1.0.2] Cryptarchia Protocol - Block Header Validation](https://nomos-tech.notion.site/Block-Header-Validation-21c261aa09df810cb85eff1c76e5798c?pvs=24#21c261aa09df810bb539f80ba66dba13).
...
1. Mempool Transactions Validation
    mempool_transactions must refer to a valid sequence of Mantle Transactions from the mempool. Each transaction must be valid according to the rules defined in the [[1.0.0] Mantle](https://nomos-tech.notion.site/1-0-0-Mantle-21c261aa09df810c8820fab1d78b53d9?pvs=24). In order to verify ZK proofs, they are batched for verification as explained in [Batch verification of ZK proofs](https://nomos-tech.notion.site/Batch-verification-of-ZK-proofs-2e3261aa09df8008b50fc06a0730f7d4?pvs=24#2e3261aa09df80b08e90f3ca9f09dfcb) to get better performances.
1. Rewards Validation
    1. Check if the first reference matches a deterministically derived Service Rewards Distribution Transaction that distributes previous session service fees as defined in [[1.0.0] Service Reward Distribution Protocol](https://nomos-tech.notion.site/1-0-0-Service-Reward-Distribution-Protocol-211261aa09df816d92c8c6ec00c6e227?pvs=24). It should take no input and output up to service_count * 4 reward notes distributed to the correct validators.
...
1. Execute the Mantle Transactions included in the block sequentially, using the execution rules defined in the [[1.0.0] Mantle](https://nomos-tech.notion.site/1-0-0-Mantle-21c261aa09df810c8820fab1d78b53d9?pvs=24).
...
1. For each sample the verifier follow the classic cryptographic verification procedure as described in [NomosDA Cryptographic Protocol - Verification](https://nomos-tech.notion.site/Verification-1fd261aa09df816fa97ac81304732e77?pvs=24#1fd261aa09df81d0b99ff545fd3f621d) except the last step, once the verifier has a single commitment $C^{(i)}$, an aggregated element $v^{(i)}$ at position $u^{(i)}$ and one proof $\pi^{(i)}$ for each sample.
1. The verifier draws a random value for each sample $r_i \overset{{\scriptscriptstyle\$}}{\leftarrow} \mathbb{F}_p$.
...
1. The verifier draws one random value for each proof $r_i \overset{{\scriptscriptstyle\$}}{\leftarrow} \mathbb{F}_p$.
1. The verifier computes:
...
The verifier follows the same procedure as in [Proofs of Claim](https://nomos-tech.notion.site/Proofs-of-Claim-2e3261aa09df8008b50fc06a0730f7d4?pvs=24#2e3261aa09df804dbe86f3487f27e6eb) but with the Groth16 proofs of ZkSignatures.
