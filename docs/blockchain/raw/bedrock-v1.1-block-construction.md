...
9. If every reference resolves to a mempool transaction and the resolved set reproduces `header.block_root`, the block proposal is reconstructed and proceeds to further validation steps; otherwise the validator rejects it, which is a local and provisional outcome rather than a finding of invalidity — see [Reference Resolution](#reference-resolution).
...
A reference resolving to no local transaction means the referenced transaction has not reached this validator's mempool, which the transaction maturity assumption above is designed to prevent; the proposal is rejected.

That rejection is **local and provisional**. It records that this validator could not reconstruct the proposal from its own mempool, not that the block is invalid: the same block may be perfectly valid and reconstruct immediately at a validator whose mempool is complete. A validator must therefore not record a reconstruction failure as a verdict on that `block_id`. The block may arrive again later — as a re-received proposal once the missing transaction has propagated, or in full, with its transactions, through ordinary chain synchronisation — and must then be validated on its merits. A validator that has not reconstructed a block cannot build on it, since it cannot establish the block's correctness, so it continues from its current tip until the block is obtained.

This is what distinguishes reconstruction failure from the failures in [Block Proposal Validation](#block-proposal-validation) that do not depend on the mempool — a frame that does not decode, a bad signature, an invalid proof of leadership. Those are properties of the proposal itself, identical at every node, and rejection for them is final.
