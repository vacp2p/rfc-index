...
- Reconstruction must not be the first expensive step. It begins with a full mempool rehash under the proposal's key, so it should follow signature and PoL verification, and an unauthenticated proposal must be discarded before any rehashing takes place. Because the key covers $`(\textit{parent}, \textit{slot}, \textit{leader})`$ rather than the whole proposal, every variant an equivocating leader can produce within one slot shares a key, and a validator that caches the index against the key pays for the rehash once per leader however many variants arrive. Implementations **should** cache it so, and **should** additionally bound reconstruction attempts per slot; `MAX_RECONSTRUCTIONS_PER_SLOT = 3` keeps the total within the one-second budget of [Reference Resolution](#reference-resolution) on validator-class hardware even where the cache is not implemented.
