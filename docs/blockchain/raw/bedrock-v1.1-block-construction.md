...
The key is **derived, not carried**. Every field of its preimage is part of the signed header, so the key is authenticated by `signature` and by `block_id` exactly as the header is, adds no bytes to the wire, and cannot be tampered with separately from the header. The preimage deliberately excludes `body_root`, so the key is fixed before transaction selection begins and the builder can compute short IDs while selecting.

It takes `leader_key` and `entropy_contribution` rather than the whole `proof_of_leadership`, and the omission of `proof` is deliberate. Groth16 proofs are **re-randomisable**: from a valid $`(A, B, C)`$ anyone holding the proof can derive a different valid proof of the same statement, without the witness and without re-proving. Absorbing `proof` into the preimage would therefore let a leader mint unlimited distinct keys for one slot at negligible cost, and with them unlimited distinct `block_id` values, each obliging every validator to repeat the mempool rehash of [Reference Resolution](#reference-resolution). The two fields used instead are **public inputs** to the proof of leadership ([Block Header Validation](cryptarchia-v1-protocol.md#block-header-validation)), so altering either changes the statement and demands a genuinely fresh proof rather than a re-randomisation — and re-randomisation is precisely the operation that leaves the statement unchanged.

The key is therefore one per $`(\textit{parent}, \textit{slot}, \textit{leader})`$, not one per proposal. A leader equivocating within its slot produces proposals that share a key, so a validator may cache the short-ID index against the key and reuse it across every variant it sees, which reduces the cost of equivocation from one rehash per proposal to one per leader. And because the preimage contains the `ProofOfLeadership`, the key of a future proposal is unknowable to anyone but its — still secret — leader until the proposal is broadcast; [Short ID keying and collision resistance](#short-id-keying-and-collision-resistance) establishes why that is what makes 8-byte references safe.
...
- Reconstruction must not be the first expensive step. It begins with a full mempool rehash under the proposal's key, so it should follow signature and PoL verification, and an unauthenticated proposal must be discarded before any rehashing takes place. Because the key covers $`(\textit{parent}, \textit{slot}, \textit{leader})`$ rather than the whole proposal, every variant an equivocating leader can produce within one slot shares a key, and a validator that caches the index against the key pays for the rehash once per leader however many variants arrive. Implementations **should** cache it so, and **should** additionally bound reconstruction attempts per slot at `MAX_RECONSTRUCTIONS_PER_SLOT = 3`. The two measures are not interchangeable, and the cache is the one that carries the budget. Measured on validator-class hardware, three variants cost about 810 ms single-core with a shared index and 380 ms across four cores, both inside the one-second budget of [Reference Resolution](#reference-resolution); rebuilding the index for each costs about 2430 ms single-core and 1130 ms across four, and **neither fits**. A node that does not share the index therefore misses the budget at this bound on any core count, while one that does absorbs sixteen variants for the same 810 ms it spends on one — the cost being flat in the number of variants rather than linear. The bound exists to stop unbounded work, not to make three affordable.
...
**After the proposal is broadcast, the window is seconds and the birthday advantage is gone.** Once the key is public, only collisions against the $`N`$ *referenced* short IDs matter: a pair of transactions colliding with each other but with no reference is indexed and never consulted, because resolution looks up exactly the $`N`$ values the proposal carries. Hitting a fixed set of $`N`$ targets is a targeted search costing $`2^{64}/N`$ attempts, and every attempt must be a distinct valid transaction, so each costs at least one `mantle_txhash` evaluation of a fresh candidate. At $`N = 1024`$ that is $`2^{54}`$ transaction hashes: two to six months on a single GPU, or fifteen to forty-five hours against a hundred of them, depending on the size of the candidate transactions.

That is measured against a window which can be bounded rather than merely called short. A ground transaction is useless unless it reaches a validator's mempool before that validator reconstructs, so the attacker's budget is the time for the proposal to complete its final broadcast — ordinary gossip, once it has left [Blend](blend-protocol.md) — plus the reconstruction it must beat, and this holds even for an adversary who is the *first* recipient. At a gossip mesh degree of 8 that broadcast takes $`\lceil \log_8 |\mathcal{V}| \rceil`$ hops over a validator set $`\mathcal{V}`$, so with the one-second reconstruction budget of [Reference Resolution](#reference-resolution) the window runs from about 1.4 s at a thousand validators to 1.7 s at a million, taking 100 ms per hop. It is almost independent of the size of the network, because hop count grows logarithmically while the reconstruction bound dominates, and what dependence there is runs the wrong way: a larger network delivers to some validators later, which widens the opportunity slightly rather than closing it. Demanding several collisions rather than one does not change this materially, because the grind is linear in their number: fourteen of them, the count needed to breach `MAX_RECONSTRUCTION_COMBINATIONS`, costs nine to twenty-six days on a hundred GPUs against a window of seconds.

The prefix length is what makes the attack infeasible, and it is worth being precise about why the window does not do that work by itself. Bounding the window shifts every prefix length by the same factor, so it cannot change the ratio between them; what it fixes is where the absolute threshold falls. Within a 1.5 s window, breaching the bound at the eight-byte prefix specified here needs of the order of $`5 \cdot 10^{7}`$ commodity GPUs, more than are manufactured. At a six-byte prefix it needs roughly 770 of them, or some eight Blake2b ASICs of the kind built for mining — rentable, and repeatable every slot, since each proposal carries a fresh key and a fresh window. For a six-byte prefix to demand even a million GPUs the window would have to close within about 1.2 ms, three orders of magnitude inside a single gossip hop. The dynamics of propagation therefore narrow the attack but do not decide it; the length of the reference does.
...
That factor does not carry to the phase it sits in. Measured end to end on the same host, rehashing and indexing $`10^6`$ mempool entries costs 652 ms, of which the hash is 217 ms and building the index the remaining 435 ms. Substituting the faster function would bring the phase to roughly 469 ms — a factor of **1.39**, not 6.4 — because two thirds of the work is the map rather than the hash, and the map degrades worse on a modest core than the hash does.
