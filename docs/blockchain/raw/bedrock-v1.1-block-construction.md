...
- Reconstruction must not be the first expensive step. It begins with a full mempool rehash under the proposal's key, so it should follow signature and PoL verification, and an unauthenticated proposal must be discarded before any rehashing takes place. Because the key covers $`(\textit{parent}, \textit{slot}, \textit{leader})`$ rather than the whole proposal, every variant an equivocating leader can produce within one slot shares a key, and a validator that caches the index against the key pays for the rehash once per leader however many variants arrive. Implementations **should** cache it so, and **should** additionally bound reconstruction attempts per slot at `MAX_RECONSTRUCTIONS_PER_SLOT = 3`. The two measures are not interchangeable, and the cache is the load-bearing one. On validator-class hardware three variants cost about 690 ms single-core with the cache and 260 ms across four cores, both inside the one-second budget of [Reference Resolution](#reference-resolution); without it they cost about 1970 ms single-core, which is not, and 720 ms across four cores, which is. A node that does not cache therefore meets the budget at this bound only by threading, and a node that does cache absorbs of the order of twenty variants single-core before the budget binds — so the bound exists to stop unbounded work, not to make three affordable.
