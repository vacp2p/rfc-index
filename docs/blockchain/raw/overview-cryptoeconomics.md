...
The pool is funded from fees rather than from block rewards because the two scale differently with usage, and because the block reward's own sensitivity to usage changes over the network's life. A block reward blends a release from the reserve, which is capped whatever the level of activity, with the average of the fees the rewards pool has collected, in the proportion the emission rate factor sets; early on the released part dominates and the reward sits near that cap, and only as the network matures does the reward track the fee flow. A share of the block reward would therefore hold the reward per claim near a fixed ceiling through the period in which the network is growing, while the fee a claim must itself pay would keep rising with the traffic — and above some level of usage the claim would cost more than it pays. A share of the fees moves with the same quantity that sets the fee at every point in that life, so the ratio between the two is a function of the parameters rather than of how busy the network happens to be.

### Who bears the cost of the diversion

The cost is not borne by the same parties at every point in the network's life: the parameter is one choice with two consequences.

Nothing is created and nothing is destroyed. [Block Rewards](block-rewards.md) counts this pool as a fourth controlled stock, alongside the circulating supply, the rewards pool and the reserve. The diversion only moves tokens into it that would otherwise have reached the rewards pool; the reserve is never drawn on to refill it, and no claim can be paid that the pool does not already hold.
...
The incidence therefore follows the regime. Early, while the reward is dominated by the reserve release, the reward itself barely moves; what changes is the route by which the diverted tokens return to circulation, through claims rather than through block rewards. In the mature network, where the reserve release approaches zero and the reward settles at the average pooled fees, a share diverted is a share not distributed: **the cost falls on the Blend service and the leaders**, in the 60/40 proportion in which they divide the block reward. That proportion is itself unchanged; what moves is the total they divide.

The emission rate factor is itself computed in part from the pooling rate, so the released part responds to the diversion as well. That response vanishes wherever the factor rests against either of its bounds, which is where both regimes above sit: pinned at its maximum the reward is the release cap and does not move, and pinned at its minimum the reward is the fee average and the factor has no room to fall further. It appears only in the band between them, and there it is not a small correction — it deepens the reduction while the fee average is below the per-block release cap and offsets it above, by an amount that near the middle of the band is comparable to the direct effect itself.

The pool is therefore a further claim on the same flow that funds the privacy layer and consensus, and `POW_SHARE` must be chosen on that basis.
