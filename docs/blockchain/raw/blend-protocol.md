...
| 1.2.0 | Add the proof of work quota and the Blend difficulty, verify the proof of quota before relaying any message, and add transactions as a data message payload; re-derive the nullifier cache bound and align the nullifier retention period | 2026-08-10 |
...
This is the quota that admits participants holding neither stake nor a declaration, so the cost of reaching the Blend network without a prior relationship to the protocol is one puzzle solution per message. That cost is set entirely by $`d_{blend}`$, whose baseline is chosen in [Blend Difficulty](bedrock-v1.1-mantle-specification.md#blend-difficulty) so that one solution is around a minute and a half of work on a single core of a current machine. A participant with no stake and no declaration can therefore send on the order of nine hundred messages a day using one core, which is the sense in which this branch is an on-ramp. The figure is measured rather than estimated, but on a desktop-class processor rather than on the small single-board computers the network is intended to run on, which are several times slower per core; it should be re-derived against the target hardware before it is relied upon.
...
It is a per epoch value, held constant for the whole epoch and identical for every proof produced within it. Both properties are required rather than incidental: the threshold is a public input to the proof, so a value that varied between provers, or within an epoch, would partition proofs into distinguishable classes and reveal which participants had produced which messages. Holding it constant for the epoch is what keeps the proof of work branch indistinguishable from the other two.
...
The reason for the reversal is that deferral makes relaying free for the sender and expensive for everyone else. A node that emits messages carrying syntactically well-formed but cryptographically invalid proofs pays nothing, while every node the message reaches pays a proof verification to discard it. That was tolerable while every participant able to inject a message had first locked a minimum stake through the SDP, because such a participant is identifiable and has something to lose. The proof of work branch removes that precondition: it admits participants that hold no stake, are not declared, and have no prior relationship with the protocol at all. Verifying before relaying moves the cost back to the first recipient, which is one node rather than the whole network, and bounds it to a single verification per message accepted.
