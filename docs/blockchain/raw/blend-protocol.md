...
This is the quota that admits participants holding neither stake nor a declaration, so the cost of reaching the Blend network without a prior relationship to the protocol is one puzzle solution per message. That cost is set entirely by $`d_{blend}`$, whose baseline is chosen in [Blend Difficulty](bedrock-v1.1-mantle-specification.md#blend-difficulty) so that one solution is on the order of a minute of work on a single core of an ordinary machine. A participant with no stake and no declaration can therefore send on the order of a thousand messages a day using one core, which is the sense in which this branch is an on-ramp. The figure rests on the throughput of the hash used to derive the ticket, which has not been measured.
