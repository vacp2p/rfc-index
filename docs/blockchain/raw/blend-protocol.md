...
2. If the payload contains transactions, then they are submitted to the node's mempool;
...
Transactions extracted from a data message are not broadcast by this protocol. They are submitted to the mempool of the node that received them, and the mempool disseminates them as its own rules specify.
...
2. If decapsulation succeeds, the decrypted *blending header* is processed:
    ...
    3. If the *last flag* of the decrypted *blending header* is turned on, **the message is completely decapsulated.** The *payload* can then be processed according to the [Payload Formatting](payload-formatting.md):
        ...
        2. If the type is transactions, submit each transaction to the mempool; a payload that does not parse as the sequence of transactions defined in [Payload Formatting](payload-formatting.md) is discarded.
...
2. If the decapsulation is successful, then:
    ...
    3. If the last flag is set ($`\Omega == 1`$) then examine the header type of the payload as defined in the [Payload Formatting](payload-formatting.md), then:
        ...
        2. If the payload is transactions, then the payload structure is verified and each transaction is submitted to the node's mempool, which disseminates it as its own rules specify. As with a block proposal, only the structure is checked here; whether a transaction is valid against the ledger is decided by Mantle and is out of scope for this protocol. The transactions must fit together within the fixed payload body defined in [Payload Formatting](payload-formatting.md), so a transaction that does not fit alone is not sendable over this protocol at all.
...
- As soon as a **data** message carrying a block proposal is generated, one random unreleased (future) **cover** message must be removed from the release schedule to maintain the node’s statistical indistinguishability. A data message carrying transactions removes no cover message.
