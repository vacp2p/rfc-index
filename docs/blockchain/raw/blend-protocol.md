...
This is the quota that admits participants holding neither stake nor a declaration, so the cost of reaching the Blend network without a prior relationship to the protocol is one puzzle solution per message. That cost is set entirely by $`d_{blend}`$, whose baseline is chosen in [Blend Difficulty](bedrock-v1.1-mantle-specification.md#blend-difficulty) so that one solution is around fifty seconds of work on a single core of the target machine, a Raspberry Pi 5. A participant with no stake and no declaration can therefore send on the order of seventeen hundred messages a day using one core, which is the sense in which this branch is an on-ramp. The figure is measured on the target hardware itself, with the reference basis fixed at one core of it.

The puzzle is not the only per message cost. Each of the $`\beta_{max}`$ encapsulations carries its own proof, and those proofs must be generated whatever the quota is. The work required by $`d_{blend}`$ is therefore an addition to a cost floor that already exists, and calibration should account for both rather than treating the puzzle as the whole price of admission.
