...
- *Data message* is a message whose payload carries content the network must act on. Two kinds are defined: a block proposal, generated by a consensus leader, and one or more Mantle transactions, generated by any participant. Any data message is indistinguishable from any other message until it is fully processed by the network. The privacy of the sender of a data message is what the protocol aims to protect.
...
- *Broadcasting* is the process of sending a data message payload (a block proposal or transactions) to all Logos Blockchain nodes.
...
1. The payload is a block proposal or transactions, then a data message is generated.
...
1. If the payload contains a block proposal or transactions, then it is broadcast to the entire Logos Blockchain network;
2. Otherwise, the message is discarded.

### Broadcasting

Broadcasting is a process of delivering the content extracted from a data message — a block proposal or transactions — to the entire Logos Blockchain network by the core node that received it. Only content that is constructed correctly can be broadcast. Any badly constructed content must be rejected. The logic behind the verification of the content is out of the scope of the Blend protocol. It is defined by the Logos Blockchain Bedrock ([\[Overview\] Bedrock Architecture](bedrock-architecture-overview.md)).

Transactions are additionally submitted to the mempool of the broadcasting node, so that they can be included in a block. Broadcasting alone would deliver them to the network but would not make them available to the node that received them for block building.
...
3. A node has one or more Mantle transactions to submit and holds an unused quota allowance. The payload of the message is those transactions.
...
2. If decapsulation succeeds, the decrypted *blending header* is processed:
    ...
    3. If the *last flag* of the decrypted *blending header* is turned on, **the message is completely decapsulated.** The *payload* can then be processed according to the [Payload Formatting](payload-formatting.md):
        ...
        2. If the type is transactions, add each transaction to the mempool and the payload to the broadcasting queue.
...
1. The payload is checked to be structurally well formed for its declared type: it must parse as a block proposal or as the sequence of transactions defined in [Payload Formatting](payload-formatting.md). A payload that fails this check is discarded. Whether the parsed content is *valid* — whether the proposal or a transaction would be accepted — is not evaluated here; that judgement belongs to the consensus and mempool logic the content is handed to in the next step.
2. The content is extracted from the payload and is broadcasted to the Logos Blockchain broadcasting channel after a random delay. Transactions are additionally submitted to the node's mempool.
...
2. A node has one or more Mantle transactions to submit and holds an unused quota allowance. The payload of the message is those transactions.
...
2. If the decapsulation is successful, then:
    ...
    3. If the last flag is set ($`\Omega == 1`$) then examine the header type of the payload as defined in the [Payload Formatting](payload-formatting.md), then:
        ...
        2. If the payload is transactions, then the payload structure is verified, each transaction is submitted to the node's mempool, and they are broadcast as defined in the [Broadcasting](#broadcasting) section. As with a block proposal, only the structure is checked here; whether a transaction is valid against the ledger is decided by Mantle and is out of scope for this protocol. The transactions must fit together within the fixed payload body defined in [Payload Formatting](payload-formatting.md), so a transaction that does not fit alone is not sendable over this protocol at all.
...
1. The content — a block proposal or transactions — is extracted from the payload.
