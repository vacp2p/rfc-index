...
| 1.3.0 | Derived the connection monitoring thresholds from the traffic a connection is expected to carry, and restated the connectivity maintenance logic around two principles: a ban requires misbehavior attributable to an authenticated identity, and the connection cap is hard. Verification of the proof of quota moved onto the relay path, so a message is relayed only after its public header is verified; the traffic a connection carries is now counted against the neighbor's identity for the epoch rather than against the connection; the rate at which connections with edge nodes are accepted is bounded, since their identities are ephemeral and their messages cost a proof verification; and the slot held by an unhealthy connection can be reclaimed at the cap. | 2026-08-26 |
...
- *Data message* is a message whose payload carries content the network must act on. Two kinds are defined: a block proposal, generated by a consensus leader, and a Mantle transaction, generated by any participant. Any data message is indistinguishable from any other message until it is fully processed by the network. The privacy of the sender of a data message is what the protocol aims to protect.
...
- *Edge node* is a Logos Blockchain node that is not a core node. Edge nodes connect to core nodes through which they send messages (block proposals or transactions).
- *Block proposer node* is a core or edge node that is generating a new data message carrying a block proposal.
...
Nodes monitor connections with their neighbors by verifying every message they receive and by counting the messages each connection carries.

1. If a message is badly constructed, or the number of messages is above a certain acceptable level, then the connection with that neighbor must be closed, the neighbor must be blacklisted, and a new connection with a randomly selected core node must be established. Both are attributable to the neighbor, as the connection authenticates it.
2. If the number of messages is below a certain acceptable level, then nothing is held against the neighbor, as it may itself be under attack. The connection stops counting as a healthy one, and if that leaves the node below its minimum peering degree, a new connection with a randomly selected core node must be established to make up for it.

A node never exceeds its maximum number of connections. The above logic enables a node to maintain the quality of the network by disconnecting from abusive and malicious nodes, while tolerating unresponsive ones, whose behavior it cannot distinguish from that of a node under attack.

## Messages

The protocol defines two types of messages: data, and cover.

- Data messages contain a meaningful payload, which is used by the Logos Blockchain network to advance the Bedrock consensus. The mechanisms that trigger data message generation are external to the Blend Protocol: the leader election process of the Bedrock consensus for a block proposal, and a participant submitting a transaction.
...
1. The payload is a block proposal or a transaction, then a data message is generated.
...
1. **Message generation**: a node must be motivated to generate messages according to the protocol, where generation means that a new message is created that is not the outcome of the processing. This is especially important for cover messages. Data messages already incentivize the node that is generating them: a block proposal rewards its sender directly through the consensus-defined rewarding mechanism ([Anonymous Leaders Reward Protocol](bedrock-anonymous-leaders-reward.md)), and a transaction is sent because its sender wants it included in a block.
...
3. It starts opening new connections.
    1. It selects at random (without replacement) a node from the set of core nodes, excluding the identities that are on its blacklist ([Connectivity Maintenance](#connectivity-maintenance)). The blacklist is not cleared at an epoch boundary, and neither selection nor acceptance of a connection may bypass it.
...
4. It starts accepting incoming connections and maintaining all connections as defined in **Maintenance**.
    ...
    2. It can accept connections with edge nodes at the rate of the $`\Lambda_E`$ parameter: [Core Node Parameters](#core-node-parameters). Since an edge connection lasts at most $`T_E`$, that rate is also what bounds how many of them are open at once.
...
A core node counts the messages carried by each of its connections, according to the [Connectivity Maintenance](#connectivity-maintenance):

- A connection carrying more messages than the maximum is spammy. It is closed, its neighbor is blacklisted, and a new connection with a randomly selected core node is established in its place to maintain the minimum peering degree.
- A connection carrying a message that fails header verification, or failing at the transport level, is closed and its neighbor is blacklisted, in the same way as a spammy one.
- A connection carrying fewer messages than the minimum is unhealthy. It is neither closed nor blacklisted, and the node establishes an additional connection only if it is left below its minimum peering degree.
- The number of connections with core nodes never exceeds the maximum. A node that reaches the maximum while still short of healthy connections adds a message to the logs informing about this situation, and may replace its least active connection, at most once per observation window.
- A connection with an edge node is closed as soon as the edge node has sent its message, or once the time given to it to do so has elapsed.
...
3. A node has a Mantle transaction to submit and holds an unused quota allowance. The payload of the message is the transaction.
...
- $`\lceil M_1 \rceil^W`$ denote the maximal number of messages a single connection may carry during an observation window $`W`$;
...
- $`\kappa_{max}`$ denote the divergence controller, which relaxes the expected traffic into the maximum above;
- $`\Lambda_E`$ denote the maximum number of connections with edge nodes a core node accepts per round;
...
- $`\kappa_{max}=6`$, the divergence controller, as derived in [Expected Connection Traffic](#expected-connection-traffic).
- $`\lceil M_1 \rceil^W = \kappa_{max} \cdot F_1 \cdot W = 558`$ and $`\lfloor M_1 \rfloor^W = \lceil F_1 \cdot W / 10 \rceil = 10`$, the maximum and the minimum number of messages per connection during the observation window, as derived in [Expected Connection Traffic](#expected-connection-traffic).
...
- $`\Lambda_E=8`$ connections per round, the rate at which edge connections are accepted, as derived in [Connectivity Maintenance](#connectivity-maintenance).
...
- $`T_E`$ denotes the time a core node gives an edge node to send its message before closing the connection.
- $`\Lambda_E`$ denotes the maximum number of connections with edge nodes the core node accepts per round. It bounds the verification work edge traffic can impose, independently of how many identities an adversary uses.
...
The thresholds that drive the [Connectivity Maintenance](#connectivity-maintenance) logic are derived from the traffic that a single connection is expected to carry. A received message is released to every neighbor except the one it came from ([Releasing](#releasing)), and a duplicate is recognized only after it has been received, so a connection carries, in expectation, every message that the network emits, less the fraction its neighbor first received from this node. That fraction is $`1/\Phi_{CC}`$ for a neighbor whose connections are as numerous as this node's, so the estimate below is an upper bound, which is the safe direction for a minimum that must not be crossed by an honest connection.
...
A connection is therefore expected to carry $`F_1 \cdot W = 93`$ messages during an observation window. Both bounds below are per connection: the load a node carries from its core neighbors is this figure multiplied by its number of connections with them, which is bounded by $`\Phi_{CC}^{Max}`$. Edge connections are not part of this figure, as each of them carries a single message and is then closed. Message spikes occur due to the probabilistic nature of the system, therefore the observation window must be large enough to average them out.
...
$$
\lceil M_1 \rceil^W = \kappa_{max} \cdot F_1 \cdot W = 558
$$

Two effects push the traffic of an honest connection above $`F_1 \cdot W`$, and they compound with each other. Some duplication is expected at the connection level, because a message can reach the same neighbor over more than one path before deduplication takes effect, which alone accounts for twice the expected traffic. During consensus bootstrapping, when the total stake cannot be reliably inferred, the observed $`F_D`$ is expected to significantly exceed its nominal value, likely matching $`F_C`$, which raises $`F_1`$ to $`1 \cdot 3 + 1 \cdot 3 = 6`$, a further factor of $`1.94`$. Their product, $`3.87`$, is a floor on $`\kappa_{max}`$ and not a usable value for it: at $`\kappa_{max}=3.87`$ the threshold sits exactly on the mean of honest bootstrapping traffic and half of all honest connections cross it.

The margin above that floor absorbs the message spikes described above. Honest bootstrapping traffic is $`2 \cdot 6 \cdot 30 = 360`$ messages per window, but its spread is not that of a Poisson count of $`360`$: the duplication does not generate independent arrivals, it delivers each message twice. The count is therefore $`2`$ copies of a Poisson count of $`180`$, whose standard deviation is $`2\sqrt{180} \approx 27`$ rather than $`\sqrt{360} \approx 19`$. At $`\kappa_{max}=6`$ the threshold still sits about $`7`$ standard deviations above the mean, which makes an honest connection crossing it a negligible event over the $`E`$ rounds of an epoch.

Raising $`\kappa_{max}`$ further reduces the sensitivity of the spam classification, and the relaxation is paid for in bandwidth and in the verification work of [Relaying](#relaying), since every counted message costs a public header verification. The upper bound on $`\kappa_{max}`$ is therefore the rate at which the slowest node the protocol targets can verify public headers, and the value above must be confirmed by measurement on that hardware before this revision is considered final. If the observed $`F_D`$ exceeds $`F_C`$, then $`\kappa_{max}`$ must be updated accordingly. Alternatively, $`\lceil M_1 \rceil^W`$ can be derived at runtime from the observed block proposal frequency, which removes the need for a static $`F_D`$, at the cost of a computation that must still account for the duplication described above.

> **Proof-of-work traffic:** this specification carries proof-of-stake traffic only. A future revision that admits messages authorized by proof of work must add that traffic to $`F_1`$. Such traffic cannot be bounded globally, as it depends on the message-solving capacity of the whole network, but it can be bounded per core node: if a core node may emit at most $`F_{PoW}`$ such messages per round, then a connection carries $`F_{PoW} \cdot \beta_{max} \cdot N`$ of them per round, and the maximum applies to the sum of both kinds of traffic.

### Connectivity Maintenance

A core node keeps the connections with its neighbors healthy and watches each of them for misbehavior. Two principles govern the logic below.

- **A ban requires attribution.** A neighbor is disconnected and blacklisted only for misbehavior its authenticated identity is responsible for: a message that fails header verification, a failure of the authenticated stream itself, or a message count above the maximum, all observed on its own connection. A failure of the authenticated stream means a TLS record that fails authentication, or a violation of the framing of the stream; it does not mean the loss of the connection. A connection that simply drops is not attributable, as any network between the two ends can cause it, and it is treated as silence rather than as misbehavior. The node can act on any of them with high confidence, as the TLS protocol identifies the neighbor as the true source of everything its connection carries, which rules out a replay by a third party. Traffic below the minimum never leads to a ban, as the neighbor may itself be under a denial-of-service or a censoring attack. That attack is not attributable, since an adversary can mount it without controlling either end of the connection, so acting on it would let the adversary have honest nodes excluded by the rest of the network.
- **The connection cap is hard.** The number of connections with core nodes never exceeds $`\Phi_{CC}^{Max}`$, including the connections whose handshake is still in progress. This bounds the bandwidth an adversary can force on the node. A node that has reached the cap opens no further connections, and refuses incoming ones, until a slot is released.

**Counting**

A node counts, for each of its connections with a core node, the messages received during the trailing observation window $`W`$. This count is $`M_1^W`$. Every received message is counted, including one that is later discarded as a duplicate, so that a neighbor flooding duplicates is bounded by the same maximum as any other neighbor.

The count is kept against the **authenticated identity of the neighbor** for the epoch, not against the connection. A neighbor that disconnects and reconnects resumes the count and the window it had already accumulated. The window advances only while the neighbor is connected: rounds during which there was no connection are not observations of it, and are neither counted as silence nor allowed to age earlier messages out of the window. A neighbor that loses its connection and returns therefore resumes where it left off, rather than being judged on a window it was absent for. Were the window tied to the connection, a neighbor could reset its own observation at will by reconnecting, and never accumulate a full window to be judged on. The node therefore retains the counts of the identities it has been connected to during the current epoch; the [Service Declaration Protocol](bedrock-service-declaration-protocol.md) bounds that set to the $`N`$ core nodes of the epoch, and it is discarded when the epoch ends.

**Classification**

A connection is classified from $`M_1^W`$ as follows:

1. It is **spammy** when $`M_1^W \gt \lceil M_1 \rceil^W`$.
2. It is **healthy** when $`\lfloor M_1 \rfloor^W \le M_1^W \le \lceil M_1 \rceil^W`$. Only healthy connections count towards the healthy peering degree $`h(\Phi_{CC})`$.
3. It is **unhealthy** when $`M_1^W \lt \lfloor M_1 \rfloor^W`$, which includes a connection that has carried no message at all.

The classification follows the observation window as it advances, so a connection returns to healthy as soon as its count is back within the bounds, without any marking to remove. The spammy condition is checked on every counted message and applies from the first message a connection carries.

A neighbor whose accumulated observation is shorter than $`W`$ counts as healthy until its first window completes, so that connections opened to make up for a shortage are not immediately counted as unhealthy. Because the observation accumulates against the identity rather than the connection, this grace is granted once per identity per epoch: a neighbor that reconnects resumes where it left off and does not receive it again. Otherwise a neighbor could hold a connection slot indefinitely, and stay out of reach of both the unhealthy classification and the replacement of rule 5, by disconnecting and reconnecting just before each window completed.

**Reaction**

1. A spammy connection is closed and its neighbor is added to the **blacklist**. A blacklisted identity must not be selected again, and must be refused on both outgoing and incoming connections. An entry added during an epoch $`n`$ expires when the epoch $`n+2`$ begins. The blacklist holds core nodes only, so the SDP set bounds it.
2. A connection with a core node whose authenticated stream fails, or that carries a message failing header verification, is closed and its neighbor is blacklisted. This is what marking a neighbor as malicious entails in the [Relaying](#relaying) logic. A message failing verification on a connection with an edge node is covered by rule 9 instead.
3. An unhealthy connection is never closed **as a penalty**, and its neighbor is never blacklisted, since the cause is not attributable to it. It stops counting towards $`h(\Phi_{CC})`$, which makes the node compensate for it through rule 4. Recovering the slot it occupies is a separate matter, governed by rule 5: reclaiming a resource is not a judgement on the neighbor holding it, and carries no ban.
4. When $`h(\Phi_{CC}) \lt \Phi_{CC}^{Min}`$, the node opens connections with core nodes selected uniformly at random, and without replacement, from the set returned by the SDP protocol, excluding itself, blacklisted identities and its current neighbors, until the minimum is restored or the cap is reached.
5. When the node is at the cap and any of its connections is unhealthy, it may replace one of them. The replacement is selected first, then the unhealthy connection with the lowest $`M_1^W`$, among those whose neighbor has completed an observation window, is closed without a ban, and the replacement takes the released slot. Ties are broken uniformly at random. Only unhealthy connections are eligible: a healthy one is never displaced. At most one connection is replaced per observation window, and a connection is never closed this way unless a replacement candidate is available.

    The condition is the presence of an unhealthy connection at the cap, not a shortage of healthy ones. Were it a shortage, a node whose healthy degree merely met $`\Phi_{CC}^{Min}`$ would never reclaim anything, and an adversary could hold every remaining slot up to $`\Phi_{CC}^{Max}`$ for the whole epoch by delivering nothing: never banned, because silence is not attributable, and never displaced, because the node was not short of healthy connections. Rate limiting the replacement to one per window, and never displacing a healthy connection, keeps the cost of this to an adversary that induces silence bounded and unrewarding.
6. A node that can neither open nor replace a connection logs that it is running below the minimum peering degree and continues with the connections it has. Reaching the cap may be a sign that the node is under attack, and the reaction to it is a matter of the privacy requirements of the operator, so the protocol keeps the cap and informs the operator rather than lifting it.
7. A connection with an edge node is closed as soon as the edge node has sent its message, or once $`T_E`$ has elapsed without it. Edge connections are not classified, as an edge node is closed by design. They need no separate ceiling of their own: rule 8 limits the rate at which they are accepted and each lasts at most $`T_E`$, so at most $`\Lambda_E \cdot T_E`$ of them are open at any time.
8. A core node accepts at most $`\Lambda_E`$ connections with edge nodes per round, counted once the [Neighbor Distinction Process](#neighbor-distinction-process) has identified the neighbor as an edge node, which is the first moment the two kinds can be told apart. Connections offered above that rate are closed without a ban. The limit is on the rate of acceptance rather than on the identity offering it, because an edge node is not in the set returned by the SDP protocol and its identity is ephemeral: an adversary can present a new one for every connection, so only a limit that ignores identity bounds the work it can impose.
9. An edge node whose message fails header verification has its connection closed and its identity added to the **edge quarantine**, which is a separate structure from the blacklist and not a weaker version of it. The two differ because the identities differ: a core node is named by the SDP set, which is bounded and costly to join, so its entry can be long lived and the list needs no ceiling. An edge identity is ephemeral and free to replace, so an entry expires at the end of the epoch and the structure has a fixed size, discarding the oldest first. A quarantined identity is refused for as long as its entry is retained: the core node accepts no further connection from it, and such a refusal does not consume the acceptance budget of rule 8. A rotating adversary evades the quarantine entirely; rule 8, which ignores identity, is what bounds the load.

The verification work a node must sustain follows from these limits. Its core neighbors can deliver at most $`\Phi_{CC}^{Max} \cdot \lceil M_1 \rceil^W / W = 8 \cdot 558 / 30 \approx 149`$ messages per round between them, and edge nodes at most $`\Lambda_E=8`$, so a node must verify about $`157`$ public headers per round in the worst case against roughly $`25`$ in expectation. $`\Lambda_E`$ must be chosen, together with $`\kappa_{max}`$, so that this worst case stays within the rate at which the slowest targeted node can verify public headers. Without rule 8 the edge term would be bounded only by how fast an adversary can open connections — headers arriving from a source that is neither counted nor attributable, at a rate the node does not control.

Rule 8 converts that into a bounded denial of service: an adversary spending $`\Lambda_E`$ connections per round can keep honest edge nodes from a chosen core node, which is why an edge node sends to $`\Phi_{EC}`$ of them rather than to one. That is the intended trade — an adversary can deny a node's edge service, but it cannot exhaust the node's ability to verify. The limit also applies only once the [Neighbor Distinction Process](#neighbor-distinction-process) has run, so the work of establishing a connection is not bounded by it; that work is orders of magnitude below a proof verification, which is why the limit is placed where the expensive work is rather than earlier.

> **Edge Timeout Calculation:** a block proposal message of $`34574`$ bytes ([Payload Formatting](payload-formatting.md)) takes $`34574 \cdot 8 / 10^6 \approx 0.28`$ s to transmit over a $`1`$ Mbit/s link. Allowing $`20\%`$ for framing and retransmission, and $`0.5`$ s for latency and jitter, gives $`1.2 \cdot 0.28 + 0.5 \approx 0.83`$ s, which is rounded up to $`T_E=1`$ round.

**Message identifiers**

The **message identifier** is the message proof of quota nullifier embedded in the public header of the message. It is used for signing the message and must be unique; otherwise, the message must be discarded. A message whose identifier has already been seen is a duplicate and is discarded by the [Relaying](#relaying) logic, without any penalty for the neighbor that sent it: with every node releasing messages to all of its neighbors, receiving the same identifier from several of them is the expected behavior of the protocol rather than an abuse of it. A neighbor that floods duplicates crosses $`\lceil M_1 \rceil^W`$ and is disconnected as spammy, so the node does not need to track which identifiers it has seen on which connection.

**Logging**

A node logs every connection it closes, every addition to and expiry from the blacklist, every replacement, and every period during which it runs below the minimum peering degree, each with the identity of the neighbor, the reason, and the window count where one applies.
...
The PoQ is constructed from three parts.
...
The third part of the PoQ is dedicated to the proof of work quota. The proof of the work quota ($`\pi^{K_{c}^{n}}_{Q_W}`$) is true when all of the following conditions are met:

- $`\exists~w`$: there exists a puzzle solution $`w`$ whose ticket is strictly below the Blend threshold $`d_{blend}`$ for the epoch $`e`$. The solution must be known to the prover, and the ticket is derived from it as specified in [Proof of Quota](proof-of-quota.md).
- $`K_{c}^{n} \in \mathbf K^{n,e}_h`$: the key $`K_{c}^{n}`$ is generated by the node $`n`$ for the epoch $`e`$.
- $`c \lt Q_W`$: the number $`c`$ of key nullifiers that limits the number of proofs of quota that can be generated from a single solution.

The proof of the work quota assumes:

- Public input: $`e`$, $`K_{c}^{n}`$, $`Q_W`$, $`d_{blend}`$.
- Private input: $`w`$, $`c`$.
- Public output: $`\nu_e`$.

Where $`\nu_e`$ is a PoQ nullifier and uniquely identifies the PoQ.

Finally, we use all three constrains and create a single proof of quota ($`\pi^{K_{i}^{n}}_{Q}`$). That is, the proof is true when ***any*** of the following conditions are met:
...
2. A node has a Mantle transaction to submit and holds an unused quota allowance. The payload of the message is the transaction.
3. A [Cover Message Schedule](#cover-message-schedule). The payload of the message contains random data.
