...
Execution priority fees are excluded from $`D_{1,t}`$. Although they are technically burned and later re-minted — their distribution to the block builder being delayed for privacy (cf. [Design Rationale](#design-rationale)) — they are supply-neutral transfers directed in full to the leader reward pool, whereas the block reward computed from $`R_\text{block}`$ is split between the Blend service and the leaders. Counting tips in $`D_{1,t}`$ would therefore re-mint them a second time through the $`(1-A_t) \cdot R_\text{block}`$ term — inflating the supply and overstating the burn signal — and would redirect part of their value to the Blend service.
