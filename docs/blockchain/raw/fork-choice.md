...
$s\_{gen}$ : sufficient time measured in slots to measure the density of block production with enough statistical significance.

In practice, we say  $s\_{gen} = \lfloor\frac{k}{4f}\rfloor$ , where  $f$  is the active slot coefficient from the leader lottery. (see [Theorem 2 of Badertscher et al., 2018 “Ouroboros Genesis”](https://eprint.iacr.org/2018/378.pdf))

$\textbf{common\\_prefix\\_depth}(b\_1, b\_2) \rarr (\mathbb{N},\mathbb{N})$ ​

Returns the minimum block depth at which the two branches converge to a common chain.

Examples:

$\textbf{common\\_prefix\\_depth}(b\_1, b\_2) = (0, 4)$ implies that $b\_2$ is ahead of $b\_1$ by 4 blocks

i.e. $4^{th}\text{-grandparent}(b\_2)=b\_1$ ​

![](/image/attachment%3Af0819d8a-3b5d-46bf-93e0-01b461f8ffe3%3AScreenshot_2025-08-21_at_3.39.41_AM.png?table=block&id=21b261aa-09df-8127-b31d-e4d33743d675&spaceId=8dee56ee-6a26-4946-83e5-607a431da45d&width=830&userId=&cache=v2&imgBuildSrc=requestProxiedImageUrl)

$\textbf{common\\_prefix\\_depth}(b\_2, b\_5) = (2, 3)$ would represent a forking tree like the one illustrated below

$$
2^{nd}\text{-grandparent}(b\_2)=3^{rd}\text{-grandparent}(b\_5)
$$
2nd-grandparent(b2​)=3rd-grandparent(b5​)

![](/image/attachment%3A445a5b52-3905-4371-8900-bfda3155cb8e%3AScreenshot_2025-08-21_at_3.42.56_AM.png?table=block&id=21b261aa-09df-8128-a35a-d2f0affc2031&spaceId=8dee56ee-6a26-4946-83e5-607a431da45d&width=830&userId=&cache=v2&imgBuildSrc=requestProxiedImageUrl)

$\textbf{density}(b\_i, d, s\_{gen})$ ​

Returns the number of blocks produced in the  $s$ slots following block $b\_{i-d}$ .

For example, in the following diagram, count the number of blocks produced in the $s\_{gen}$ slots of the highlighted area.

![](/image/attachment%3A47e985b3-9ed2-4cef-ae25-97327993c455%3AScreenshot_2025-05-23_at_12.02.42_PM.png?table=block&id=21b261aa-09df-81b3-a43c-d884eae8fce7&spaceId=8dee56ee-6a26-4946-83e5-607a431da45d&width=1330&userId=&cache=v2&imgBuildSrc=requestProxiedImageUrl)

We look backwards starting from $b\_i$ , looking at the $d^{th}$ grandparent of $b\_i$ . We denote this block $b\_{i-d}^{sl\_t}$ and note that it was created in slot $sl\_t$ . The density calculation considers the number of blocks created in the next $s\_{gen}$ slots. The last block in this interval is $b\_{i-d+j}^{sl\le t+s\_{gen}}$ , that is, its the last block who’s slot number is less than or equal to $s$ slots after $sl\_t$ .

ALT

### Bootstrap Fork Choice Rule

During bootstrapping, we use the Ouroboros Genesis fork choice rule (

maxvalid-bg

)

def bootstrap\_fork\_choice(c\_local, forks, k, s\_gen):
c\_max = c\_local
for c\_fork in forks:
depth\_max, depth\_fork = common\_prefix\_depth(c\_max, c\_fork):
if depth\_max <= k:
# the fork depth is less than our safety parameter `k`. It's safe
# to use longest chain to decide the fork choice.
if depth\_max < depth\_fork
...
if density(c\_max, depth\_max, s\_gen) < density(c\_fork, depth\_fork, s\_gen):
...
During normal operations, we use the Ouroboros Praos fork choice rule (

maxvalid-mc

). Here we reject any forks that diverge further back than $k$ blocks.

def online\_fork\_choice(c\_local, forks, k):
c\_max = c\_local
for c\_fork in forks:
depth\_max, depth\_fork = common\_prefix\_depth(c\_max, c\_fork):
if depth\_max <= k:
...
Specifications](https://nomos-tech.notion.site/1-0-0-Cryptarchia-Fork-Choice-Rule-21b261aa09df811584dfd362abb26627?pvs=26&qid=1:0c2f2a4c-ad8d-47d9-9d18-50d072249c39:0)
...
Nodes will remain on the honest chain if they reject forks that diverge further back than ΣEquation blocks without further inspection. In order for an adversary to succeed, they would need to build a ΣEquation-deep chain faster than the time it takes the honest nodes to grow the honest chain by ΣEquation blocks. The adversary must build this chain live, alongside the honest chain. They cannot build this chain after-the-fact since online nodes will be rejecting any fork that diverges before their ΣEquation-deep block.

Protocol

Definitions

- ΣEquation : safety parameter, i.e. the depth at which a block is considered immutable
- ΣEquation : sufficient time measured in slots to measure the density of block production with enough statistical significance.

  In practice, we say ΣEquation, where ΣEquation is the active slot coefficient from the leader lottery. (see Theorem 2 of Badertscher et al., 2018 “Ouroboros Genesis”)
- ΣEquation
  ...
  1. ΣEquation implies that ΣEquation is ahead of ΣEquation by 4 blocks

     i.e. ΣEquation

     ![](/image/attachment%3Af0819d8a-3b5d-46bf-93e0-01b461f8ffe3%3AScreenshot_2025-08-21_at_3.39.41_AM.png?table=block&id=21b261aa-09df-8127-b31d-e4d33743d675&spaceId=8dee56ee-6a26-4946-83e5-607a431da45d&userId=&cache=v2&imgBuildSrc=requestProxiedImageUrl)
  2. ΣEquation would represent a forking tree like the one illustrated below

     📈Equation

     ![](/image/attachment%3A445a5b52-3905-4371-8900-bfda3155cb8e%3AScreenshot_2025-08-21_at_3.42.56_AM.png?table=block&id=21b261aa-09df-8128-a35a-d2f0affc2031&spaceId=8dee56ee-6a26-4946-83e5-607a431da45d&userId=&cache=v2&imgBuildSrc=requestProxiedImageUrl)
- ΣEquation

  Returns the number of blocks produced in the ΣEquation slots following block ΣEquation.

  For example, in the following diagram, count the number of blocks produced in the ΣEquation slots of the highlighted area.

  ![](/image/attachment%3A47e985b3-9ed2-4cef-ae25-97327993c455%3AScreenshot_2025-05-23_at_12.02.42_PM.png?table=block&id=21b261aa-09df-81b3-a43c-d884eae8fce7&spaceId=8dee56ee-6a26-4946-83e5-607a431da45d&userId=&cache=v2&imgBuildSrc=requestProxiedImageUrl)
...
During normal operations, we use the Ouroboros Praos fork choice rule (maxvalid-mc). Here we reject any forks that diverge further back than ΣEquation blocks.
...
2. Ouroboros Praos: An adaptively-secure, semi-synchronous
