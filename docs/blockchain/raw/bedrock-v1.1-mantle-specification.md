...
| 1.6.0 | Required checked arithmetic for all token value, balance, gas, and fee computations. | 2026-07-02 |
...
def mandatory_fees(signed_tx: SignedMantleTx,
        ...
        permanent_storage_fees = checked_uint64(
        ...
        for op in mantle_tx.ops:
                ...
                tx_execution_gas = checked_uint64(tx_execution_gas + execution_gas(op))
...
for inp in deposit.inputs:
        channels[deposit.channel].balance = checked_uint64(
...
```python
withdrawal_amount = checked_uint64(sum(output.value for output in withdrawal.outputs))
...
for output in withdrawal.outputs:
        channels[withdrawal.channel].balance = checked_uint64(
