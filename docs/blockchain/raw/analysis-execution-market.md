...
- **2026-05-22** — [`e8cedab`](https://github.com/logos-co/logos-lips/blob/e8cedab986dd17a9ff7bc718398b7162c2ab0d89/docs/blockchain/raw/analysis-execution-market.md) — fix(blockchain): re-import 25 Notion-only spec bodies with proper markdown formatting
- **2026-05-22** — [`4c38737`](https://github.com/logos-co/logos-lips/blob/4c38737840346f1d3ba899ecab61c255e5c88f93/docs/blockchain/raw/analysis-execution-market.md) — chore(blockchain): import 25 Notion-only specs as scaffold files (PR #2A)
...
Here, we model the total Execution Gas used in a block, $G[s]$, as a function of the base fee, $G[b_\text{exec}[s] ]$, where we assume a standard demand curve with $G' < 0$ (demand for Execution Gas decreases as the price increases).

The system is in equilibrium when its state variables no longer change. Let the equilibrium state be $(b^*, G_{\text{avg}}^*)$. From the base fee update rule (2), for $b_\text{exec}[s+1] = b_\text{exec}[s] = b^*$, the adjustment factor must be zero. This implies:

$$
G_{\text{avg}}^* - G_{\text{target}} = 0 \implies \boldsymbol{G_{\text{avg}}^* = G_{\text{target}}}
$$

From the EMA update rule (1), for $G_{\text{avg}}[s] = G_{\text{avg}}[s-1] = G_{\text{avg}}^*$, we must have:

$$
\begin{aligned}G_{\text{avg}}^* =& (1 - q) \cdot G[b^*] + q \cdot G_{\text{avg}}^*\\
(1 - q) \cdot G_{\text{avg}}^* =& (1 - q) \cdot G[b^*]\\
\implies& {G_{\text{avg}}^* = G[b^*]}\end{aligned}
$$

Conclusion: Combining these results, the system reaches equilibrium when the execution gas demanded by the market at price $b^*$ is exactly equal to the protocol's target: ${G[b^*] = G_{\text{target}}}$. The equilibrium base fee, $b^*$, is the market-clearing price that induces a level of network activity precisely equal to the desired target.

# Base Fee Stability Analysis

Stability analysis determines if the system will naturally converge to the equilibrium state $(b^*, G_{\text{target}})$ after a market shock. Due to the two-variable, cross-dependent nature of the system, we analyze the Jacobian matrix of the linearized system around the equilibrium point.
