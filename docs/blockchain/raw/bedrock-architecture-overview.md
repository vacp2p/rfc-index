...
Channels A and B form virtual chains on top of the Logos Blockchain. Channel messages are included in blocks on the Logos Blockchain in such a way that they respect the ordering of channel messages e.g. $B\_4$ must come after $B\_3$ in the Logos Blockchain.
...
Logos BlockchainCryptarchiaLogos MempoolSovereign ZoneClientsCryptarchiaLogos MempoolSovereign ZoneClientsAlice's TxBob's TxCharlie's TxOrder, Execute and Bundle Tx's into an InscriptionInscription Mantle TransactionLeader includes transaction in next blockBlock finalizes after being buried by 2160 blocksClient observes the SR Inscription finalized (finality)
