...
The PoL mechanism ensures that a note has legitimately won the leadership election while protecting the leaders privacy. The protocol is:
...
1. Compute and output the entropy contribution $\rho_{LEAD} := \text{hash}(\text{NONCE\_CONTRIB\_V1} || sl||noteID||sk)$
