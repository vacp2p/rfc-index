...
Without loss of generality, assume that the users $u\_0$ and $u\_1$ provide the messages $m\_0$ and $m\_1$ , respectively, in a “direct” manner to $R$ (due to symmetry and independence, the “cross” case can be analysed similarly?).

We denote the following random variables:

The delay $x\_0$ until $m\_0$ is sent to TTP by $u\_0$ .

The delay $x\_1$ until $m\_1$ is sent to TTP by $u\_1$ .

The delay $y\_0$ of TTP until $m\_0$ is forwarded to $R$ , i.e. the time $m\_0$ stays in the TTP.

The delay $y\_1$ of the TTP until $m\_1$ is forwarded to $R$ , i.e. the time $m\_1$ stays in the TTP

We have that $t\_{s,0}$ , $t\_{s,1}$ , $t\_{r,0}$ and $t\_{r,1}$ are the time values of $x\_0$ , $x\_1$ , $x\_0$ + $y\_0$ , $x\_1 + y\_1$ , that adversary observes, in the direct case.

Thus adversary wins when either one of the following events happen:

$E\_{0<1}: x\_0 < x\_1$ and $x\_0 + y\_0 < x\_1 + y\_1$ ;

or $E\_{0\geq1}: x\_0 \geq x\_1$ and $x\_0 +y\_0 \geq x\_1 +y\_1$ ;
