...
| 1.1.0 | Made verifiable-checkpoint the default fast bootstrap: checkpoint at epoch e−1 with state verified against `epoch_state_root`, background backfill of older blocks, and an explicit participation gate. | 2026-06-26 |
...
The default way to bootstrap is from a recent **verifiable checkpoint**, which lets a node catch up without replaying the chain from Genesis. A checkpoint is the pair `(checkpoint block, settled state)`, where the checkpoint block is the first block of epoch $`e-1`$ (with $`e`$ the current epoch) and the settled state is the ledger and consensus state committed by that block's `epoch_state_root`. As defined in [Epoch State Root](cryptarchia-v1-protocol.md#epoch-state-root), the `epoch_state_root` of the first block of an epoch commits the state as of the last block of the *previous* epoch, after the epoch-transition logic is applied. The checkpoint block therefore commits the settled end-of-epoch-$`e-2`$ state: even though the checkpoint block itself is only one epoch back, the state it commits is a full two epochs back and hence finalized. Epoch $`e-1`$ is also far enough back that the checkpoint block sits beyond the $`k`$-deep finality boundary. The node considers blocks up to and including the checkpoint block as immutable.
...
Blocks below the checkpoint must still follow consensus. They are downloaded in the background, after the node starts participating, for archival and serving purposes. To check their consensus validity the node does not execute every transaction: it executes only the ones that change the [Eligible Leader Notes](cryptarchia-v1-protocol.md#eligible-leader-notes) and tracks each block's entropy contribution to evolve the [Epoch Nonce](cryptarchia-v1-protocol.md#epoch-nonce). That is enough to recover each epoch's lottery inputs and verify every block's [Proof of Leadership](cryptarchia-v1-protocol.md#proof-of-leadership). Each block must also chain to the checkpoint through its `parent_block` and match its `block_root`. Because the leadership proofs are Groth16, the node can verify the backfilled blocks in batches rather than one at a time (see [Batch verification of ZK proofs](bedrock-v1.1-block-construction.md#batch-verification-of-zk-proofs)), which keeps backfilling the full history cheaper.
...
A checkpoint provider serves the `GET /checkpoint` API, which lets any client download a checkpoint block and its corresponding settled state. It is aimed at users that cannot obtain one over p2p (for example, light clients or nodes not yet peered), but it is open to anyone: callers are not, and need not be, filtered by how they connect. The downloaded state is verified against the block's `epoch_state_root` (see [Bootstrapping from Checkpoint](#bootstrapping-from-checkpoint)), so the provider need not be trusted for state integrity.
