...
| Contributors | Prem Prathi <prem@status.im>, Daniel Kaiser <danielkaiser@status.im>, Hanno Cornelius <hanno@status.im>, Mohammed Alghazwi <mohalghazwi@status.im> |
...
- **2026-02-09** — [`afd94c8`](https://github.com/logos-co/logos-lips/blob/afd94c8bc1420376ae9af7e14a4feb246f2ed621/docs/ift-ts/raw/mix.md) — chore: add math support (#287)
- **2026-01-29** — [`925aeac`](https://github.com/logos-co/logos-lips/blob/925aeac395050c948c1cceb1d5b0673836ebd442/docs/ift-ts/raw/mix.md) — chore: changes to how per-hop proof is added to sphinx packet which makes it simpler (#263)
- **2026-01-19** — [`f24e567`](https://github.com/logos-co/logos-lips/blob/f24e567d0b1e10c178bfa0c133495fe83b969b76/docs/ift-ts/raw/mix.md) — Chore/updates mdbook (#262)
- **2026-01-16** — [`f01d5b9`](https://github.com/logos-co/logos-lips/blob/f01d5b9d9f2ef977b8c089d616991b24f2ee4efe/docs/ift-ts/raw/mix.md) — chore: fix links (#260)
- **2026-01-16** — [`89f2ea8`](https://github.com/logos-co/logos-lips/blob/89f2ea89fc1d69ab238b63c7e6fb9e4203fd8529/docs/ift-ts/raw/mix.md) — Chore/mdbook updates (#258)
- **2025-12-22** — [`0f1855e`](https://github.com/logos-co/logos-lips/blob/0f1855edcf68ef982c4ce478b67d660809aa9830/docs/vac/raw/mix.md) — Chore/fix headers (#239)
- **2025-12-22** — [`b1a5783`](https://github.com/logos-co/logos-lips/blob/b1a578393edf8487ccc97a5f25b25af9bf41efb3/docs/vac/raw/mix.md) — Chore/mdbook updates (#237)
- **2025-12-18** — [`d03e699`](https://github.com/logos-co/logos-lips/blob/d03e699084774ebecef9c6d4662498907c5e2080/docs/vac/raw/mix.md) — ci: add mdBook configuration (#233)
- **2025-12-15** — [`dabc317`](https://github.com/logos-co/logos-lips/blob/dabc31786b4a4ca704ebcd1105239faff7ac2b47/vac/raw/mix.md) — fixing format errors in mix rfc (#229)
- **2025-12-12** — [`4f54254`](https://github.com/logos-co/logos-lips/blob/4f54254706666e373b5987901d974dc40f109293/vac/raw/mix.md) — fix format errors in math sections for mix rfc (#225)
- **2025-12-11** — [`7f1df32`](https://github.com/logos-co/logos-lips/blob/7f1df32779d5ff3fa57abc2e71ac2ad1b9662965/vac/raw/mix.md) — chore: use sembreaks for easy review and edits (#223)
- **2025-12-10** — [`e742cd5`](https://github.com/logos-co/logos-lips/blob/e742cd519200a7cb0971936bd1dbbeb31397bce5/vac/raw/mix.md) — RFC Addition: Section 9 Security Considerations (#194)
- **2025-12-10** — [`9d11a22`](https://github.com/logos-co/logos-lips/blob/9d11a22901ffe9c3ab0007f58baa55b0e08abfc9/vac/raw/mix.md) — docs: finalize Section 8 Sphinx Packet Construction and Handling (#202)
- **2025-10-05** — [`36be428`](https://github.com/logos-co/logos-lips/blob/36be428cdd2f12e1c9a1dd0b984dc9a6fd4acfee/vac/raw/mix.md) —  RFC Refactor: Sphinx Packet Format (#173)
- **2025-06-27** — [`5e3b478`](https://github.com/logos-co/logos-lips/blob/5e3b4788ec1c0ec07535efe321b103d52b118db5/vac/raw/mix.md) —  RFC Refactor PR: Modular Rewrite of Mix Protocol Specification (#158)
- **2025-06-02** — [`db90adc`](https://github.com/logos-co/logos-lips/blob/db90adc94e9f69627dd1254159d33eb062f00867/vac/raw/mix.md) — Fix LaTeX errors (#163)
- **2024-11-08** — [`38fce27`](https://github.com/logos-co/logos-lips/blob/38fce27d33ff11fcf15f947a9985c86b297c71b6/vac/raw/mix.md) — typo fix
- **2024-09-16** — [`7f5276e`](https://github.com/logos-co/logos-lips/blob/7f5276e18c16e644ba6645d20854ce6e6dd82aad/vac/raw/mix.md) — libp2p Mix Protocol Spec Draft (#97)
...
- **Sender**
  The application-level entity that originates the origin protocol message.
...
- **Initiating Node**
  The mix node that constructs and transmits Sphinx packet(s) on behalf of the sender.
  Handles all mix-layer responsibilities, such as path selection, forwarding delay encoding, and preparation of any configured packet components.
...
- **Single-Use Reply Block (SURB)**
  ...
  SURBs are generated by the initiating node and included in the Sphinx packet sent to the recipient.
...
- **Sphinx payload ($δ$)**
  The fixed-size encrypted field $δ$ of a Sphinx packet.
  It carries the padded application message and any payload extensions such as SURBs.

- **Payload encryption key ($δ_{\mathrm{key}}$)**
  A per-hop key used to encrypt or decrypt one layer of the Sphinx payload field $δ$. 

- **Payload integrity prefix**
  A fixed all-zero byte string prepended to the plaintext payload before applying the payload encryption.
  The final hop verifies this prefix after payload decryption to detect payload tampering.
  The length and construction of this prefix are defined in the [LIONESS specification](./mix-lioness.md).
...
Sphinx packets also include per-hop integrity checks and enforce a maximum path length.
...
- Required for interoperability and path construction (_e.g.,_ discovery, delay strategy, DoS protection).
- Optional or deployment-specific (_e.g.,_ exit abuse prevention, cover traffic, incentivization).
...
The initiating node is responsible for appending the appropriate exit abuse prevention data (_e.g.,_ nonce, timestamp) to the message payload.
...
The Mix Protocol supports a pluggable DoS protection mechanism to defend the mixnet against denial-of-service (DoS) or resource exhaustion attacks.
Without DoS protection, malicious actors could flood mix nodes with valid Sphinx packets, exhausting computational resources or bandwidth of the mix nodes, eventually making them unusable.
Deployments MUST implement a DoS protection mechanism to make the mixnet robust under adversarial conditions.

The DoS protection mechanism is pluggable because different deployments may require different trade-offs between computational overhead, attack resistance, and Sybil resistance.
For a specific mix deployment, the same DoS protection mechanism MUST be followed by all participating mix nodes to ensure interoperability.

Detailed design requirements, architectural approaches, packet structure considerations, node responsibilities, and recommendations are specified in [Mix DoS Protection](mix-dos-protection.md).
...
- **Header Encryption**: AES-128 in Counter Mode (AES-CTR)
  ...
  - **Keys and IVs**: Each derived from the session key for the hop using the KDF output truncated to $128$ bits.

- **Payload Encryption**: LIONESS wide-block encryption

  - **Purpose**: To encrypt the fixed-size payload $δ$ for each hop and enable end-to-end payload-integrity.
  - **Construction**: Defined by the [LIONESS specification](./mix-lioness.md).
  - **Keys and IVs**: LIONESS internal round keys and IVs are derived using the KDF output according to the [LIONESS specification](./mix-lioness.md).
...
Implementations MUST account for payload integrity prefix size (defined in the [LIONESS specification](./mix-lioness.md)) and payload extensions, such as SURBs, when determining the maximum message size that can be encapsulated in a single Sphinx packet.

Note: In this specification, the payload size MUST be at least 64 bytes, including the payload integrity prefix. This is due to the use of LIONESS which requires the input block (payload in this case) to be at least 64 bytes. For more details see the [LIONESS specification](./mix-lioness.md).
...
3. **Wrap Plaintext Payload In Sphinx Packet**
   ...
   - For each $i$ (from $1$ to $L-1$):
     ...
     - Derive per-hop AES key and IV:
       ...
       \mathrm{KDF}(\text{"aes\_key"},  s_{i-1})\\
       Φ_{\mathrm{iv}_{i-1}} =
       \mathrm{KDF}(\text{"iv"}, s_{i-1})
       \end{array}
       `$
     ...
     - Compute the filler string $Φ_i$ using $\text{AES-CTR}^\prime_i$, which is AES-CTR encryption with the keystream starting from index $((t+1)(r-i)+t+2)κ$ :
       ...
       where $0_x$ defines the string of $0$ bits of length $x$.
   ...
   - Derive per-hop AES key, MAC key, and IV:
     ...
     \mathrm{KDF}(\text{"aes\_key"}, s_i)\\
     \mathrm{mac\_key}_i =
     \mathrm{KDF}(\text{"mac\_key"}, s_{i})\\
     β_{\mathrm{iv}_i} =
     \mathrm{KDF}(\text{"iv"}, s_i)
     \end{array}
     `$
   ...
   - Using the derived keys and encoded forwarding delay, compute the nested encrypted routing information $β_i$:
     ...
     - Otherwise (_i.e.,_ intermediary node):
       ...
       where $X_{[a \ldots b]}$ denotes the substring of $X$ from byte offset $a$ to $b$, inclusive, using zero-based indexing.
   ...
   - Derive per-hop payload encryption key:
     ...
     \begin{array}{l}
     δ_{\mathrm{key}_i} =
     \mathrm{KDF}(\text{"payload\_enc\_key"}, s_i)\\
     \end{array}
     `$
...
5. **Decrypt One Layer of the Payload**
   ...
   - Derive the payload encryption key from the session secret $s$:
     ...
     \begin{array}{l}
     δ_{\mathrm{key}} =
     \mathrm{KDF}(\text{"payload\_enc\_key"}, s)\\
     \end{array}
     `$
...
2. **Recover Padded Application Message**
   ...
   If the payload integrity prefix check fails, discard $δ'$ and terminate processing.
   ...
   - Extract rest of the bytes of $δ'$ as the padded application message $m$:
     ...
     where $X_{[a \ldots]}$ denotes the substring of $X$ from byte offset $a$ to the end of the string, using zero-based indexing.
...
2. **Recover Padded Reply Message**
   ...
   - Decrypt the final layer i.e., reversing the effect of the initial encryption in step 2 of [section 8.7.3](#873-using-a-surb):
   ...
   - Verify the decrypted payload $δ$:
...
3. **Recover Reply Message**
   ...
   - Extract rest of the bytes of $δ$ as the padded reply message $m$:
...
DoS protection&mdash;such as admission control, rate-limiting, or resource-bound access&mdash;can be plugged in as explained in [Section 6.6](#66-dos-protection).
